...
>
> 🎸 **姉妹アプリ: [Kurogane Bass](BASS.md)** … 同じくサンプルを使わない、デジタル導波管方式の
> エレキベース・アプリです（`/bass/` で公開、開発時は `npm run bass:dev`）。
> 指板をタップして演奏でき、スライド・チョーキング・スラップにも対応しています。
> 仕様と仕組みは [BASS.md](BASS.md) を参照してください。
